...
each vertex in the track (i.e., length N for N vertices in `Tracks.data`). As we
will see below, we can use the values in a property to set the display
...
width" slider in the Tracks layer controls.
...
`viewer.add_tracks()` and `napari.view_tracks()` methods. From a layer that as
...
this example
[here](https://github.com/napari/napari/blob/main/examples/tracks_3d.py)

![image: tracks 3D plus time](../../images/tracks_3d_t.gif)

## Next steps

Hopefully, this guide has given you a detailed understanding of the `Tracks`
