...
const INTERSPHINX_FILE = resolve(BUILD_DIR, 'objects.inv');
const PUBLIC_INTERSPHINX_FILE = resolve(PUBLIC_DIRECTORY, 'objects.inv');
...
export async function copyPublicFiles(): Promise<void> {
  ...
  await Promise.all([
    ...
    fs.copy(INTERSPHINX_FILE, PUBLIC_INTERSPHINX_FILE),
  ]);
}
